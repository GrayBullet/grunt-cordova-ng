  ...
  module.exports = {
    getEnvironmentOptions: function(optEnvironment) {
      ...
      appendOption(options,
                   'platforms',
                   getArrayOption(env['GRUNT_CORDOVA_NG_PLATFORMS'])); //jshintignore: line


      // GRUNT_CORDOVA_NG_BUILD -> options.build
      appendOption(options, 'build', env['GRUNT_CORDOVA_NG_BUILD']); // jshintignore: line

      // GRUNT_CORDOVA_NG_DEVICE -> options.device
      appendOption(options, 'device', env['GRUNT_CORDOVA_NG_DEVICE']); // jshintignore: line

      // GRUNT_CORDOVA_NG_TARGET -> options.target
      appendOption(options, 'target', env['GRUNT_CORDOVA_NG_TARGET']); // jshintignore: line

      return options;
    },

    getGruntArgumentsOptions: function(grunt) {
      var options = {};

      // --cordova-platforms -> options.platforms
      appendOption(options,
                   'platforms',
                   getArrayOption(grunt.option('cordova-platforms')));

      // --cordova-build -> options.build
      appendOption(options,
                   'build',
                   grunt.option('cordova-build'));

      // --cordova-device -> options.device
      appendOption(options,
                   'device',
                   grunt.option('cordova-device'));

      // --cordova-target -> options.target
      appendOption(options,
                   'target',
                   grunt.option('cordova-target'));
    ...
    mergeOptions: function(options, build, optEnvironment) {
